...
BEGIN_MESSAGE_MAP(CNLaminateAppApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CNLaminateAppApp::OnAppAbout)
...
CNLaminateAppApp::CNLaminateAppApp()
{
	// 다시 시작 관리자 지원
	...
	SetAppID(_T("NLaminateApp.AppID.NoVersion"));

	// TODO: 여기에 생성 코드를 추가합니다.
	// InitInstance에 모든 중요한 초기화 작업을 배치합니다.
}

// 유일한 CNLaminateAppApp 개체입니다.

CNLaminateAppApp theApp;


// CNLaminateAppApp 초기화
LPCTSTR lpszMutexName = TEXT("NLaminateAppMutex");
LPCTSTR lpszMsgBoxHeader = TEXT("NLaminateApp");

BOOL CNLaminateAppApp::InitInstance()
{
	...
	if (!FirstInstance())
	{
		MessageBox(NULL, TEXT("  NLaminateApp is already activated ! "), lpszMsgBoxHeader, MB_OK | MB_ICONEXCLAMATION);
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CNLaminateAppDoc),
		RUNTIME_CLASS(CMainFrame),       // 주 SDI 프레임 창입니다.
		RUNTIME_CLASS(CNLaminateAppView));
	if (!pDocTemplate)
		return FALSE;
...
BOOL CNLaminateAppApp::FirstInstance()
{
...
int CNLaminateAppApp::ExitInstance()
{
	if (m_pDlgSplash)
	{
...
HWND CNLaminateAppApp::GetSplashHwnd()
{
...
void CNLaminateAppApp::SetInitFinished()
{
	if (m_pDlgSplash)
	{
...
int CNLaminateAppApp::ExecMessageHandleAll()
{
	int returnValue;
	MSG Mess;
...
void CNLaminateAppApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}
...
// CNLaminateAppApp 메시지 처리기
BOOL CNLaminateAppApp::PreTranslateMessage(MSG* pMsg)
{
	BOOL bCtrl;
